...
startup failure 如果发生在 ready 之前，只通过 stderr + non-zero process exit 表达，不映射为 runtime lifecycle event。
...
成功 `spawn` 后发生的 ChildProcess `error` 不再定义为 `spawnFailed`；它作为 diagnostics/error path 处理。
...
Control Plane 在 shutdown convergence 完成前保持可用；RPC 是最后关闭的 Hostra-owned runtime resource。
...
当 window registry 为空且 `subprocess === null` 时，shutdown convergence 完成，可以立即关闭 RPC 并退出，无需等待 grace timer。
...
- `closeWindow()` 返回语义保持不变；
- `getAllWindows()` 保留现有字段，并允许 additive 增加 `webContentsId`；
- RPC token 鉴权方式保持不变；
...
显式 `HOSTRA_ELECTRON_VERSION` override 单独验证。
...
- 读取 package pinned Electron version 的安装结果；
- 继续 spawn Electron with inherited stdio；
