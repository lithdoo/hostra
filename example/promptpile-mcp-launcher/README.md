...
- 本仓库 **`packages/promptpile-mcp`** 已执行 **`npm install`**（会执行 `prepare` 生成 **`dist/`**；若直接运行提示缺少 `dist/src/index.js`，请先在该目录安装依赖）。
...
**终端 B**（在任意目录，确保能解析到本仓库的 `packages/promptpile-mcp`）：

```bat
cd /d path\to\hostra\example\promptpile-mcp-launcher
npx --prefix "..\..\packages\promptpile-mcp" promptpile-mcp export-tools --base-url http://127.0.0.1:8765
```
