...
```env
HOSTRA_APP_NAME=my-electron-app
HOSTRA_RPC_PORT=9333
HOSTRA_RPC_TOKEN=replace-with-strong-token
...
- `HOSTRA_APP_NAME`：应用名（可选）。
- `HOSTRA_RPC_PORT`：RPC 端口（默认 `9333`）。
- `HOSTRA_RPC_TOKEN`：RPC 鉴权令牌（可选，设置后客户端必须携带 token）。
- `HOSTRA_SUBCMD`：子进程命令（可选，子进程退出后应用退出）。
- `HOSTRA_CONFIG_DIR`：配置目录（默认 `process.cwd()`，用于相对路径解析）。
- `HOSTRA_USER_DATA_DIR`：Electron `userData` 目录（可选）。设置后可精确控制 cookie/session 本地持久化根目录。
- `HOSTRA_MIRROR`：Electron 下载镜像（安装时使用）。
...
也就是说，你在 `.env` 中的 `HOSTRA_*` 或其他业务变量，子进程都能直接读取。
...
```text
ws://localhost:<HOSTRA_RPC_PORT>
```

启用 token 后：

```text
ws://localhost:<HOSTRA_RPC_PORT>?token=<HOSTRA_RPC_TOKEN>
```

未配置 `HOSTRA_RPC_TOKEN` 时保持兼容：不做 token 校验。  
...
- 其它非空字符串：视为相对 `HOSTRA_CONFIG_DIR` 的本地路径，先 `path.resolve`，再转为 `file://…` 加载。
...
- 下载失败：检查网络，必要时设置 `HOSTRA_MIRROR`。
- 连接失败：确认端口、进程是否运行、token 是否正确。
- 相对路径打不开：确认 `HOSTRA_CONFIG_DIR` 指向正确目录。
